...
def build_model(args, only_teacher=False, img_size=224, device=None):
    if "vit" in args.arch:
        vit_kwargs = dict(
            ...
            register_to_register_attention=getattr(args, "register_to_register_attention", False),
            patch_to_patch_attention=getattr(args, "patch_to_patch_attention", True),
            slot_start_layer=getattr(args, "slot_start_layer", 0),
            register_budget_gate=getattr(args, "register_budget_gate", False),
            register_init=getattr(args, "register_init", "learned"),
            register_insert_layer=getattr(args, "register_insert_layer", None),
            ...
            register_orth_eps=getattr(args, "register_orth_eps", 1e-6),
            register_orth_preserve_norm=getattr(args, "register_orth_preserve_norm", True),
            covariance_mode=getattr(args, "covariance_mode", "none"),
            covariance_space=getattr(args, "covariance_space", "value"),
            covariance_normalization=getattr(args, "covariance_normalization", "relative_second_moment"),
            covariance_eps=getattr(args, "covariance_eps", 1e-6),
            covariance_gate_strength=getattr(args, "covariance_gate_strength", 0.6931471805599453),
            device=device,
        )
